...
class rqlConnector {
    ...
    constructor(_baseHref, _connectorMode) {
        ...
        this.info = {
            package: {
                name: `{build-package}`,
                extensions: `{build-package-extensions}`,
                information: `{build-package-information}`,
...
function createRqlConnector(requestParam, callbackFunc) {
    ...
    if (Cookies.get(`rql.connector.js-{build-package}`)) {
        var connectorCookie = JSON.parse(Cookies.get(`rql.connector.js-{build-package}`));
    } else {
        var connectorCookie = {
            release: `0.0.0.0`
        }
    }
    if (connectorCookie.release == `{build-release}`) {
                                                    ...
                                                    getServerVersion(
                                                        ...
                                                        function (responseData) {
                                                            ...
                                                            setProgress({
                                                                ...
                                                                function () {
                                                                    Cookies.set(
                                                                        `rql.connector.js-{build-package}`, {
